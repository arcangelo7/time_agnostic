...
class DatasetBuilder(object):
        ...
        if "reference" in item:
            for reference in item["reference"]:
                if "DOI" in reference:
                    ...
                    reference_be = graphset.add_be(self.resp_agent, self.source)
    ...
    def generate_graph(self, journal_data_path:str) -> GraphSet:
        ...
        if journal_item is not None:
            journal_br = journal_graphset.add_br(self.resp_agent, self.source)
            journal_br.create_journal()
            journal_id = journal_graphset.add_id(self.resp_agent, self.source)
            ...
            if "issn-type" in journal_item:
                for issn_type in journal_item["issn-type"]:
                    ...
                    DatasetBuilder._manage_resource_embodiment(journal_graphset, journal_item, journal_br, digital_format, self.resp_agent, self.source)
        else: 
            journal_br = None
        pbar = tqdm(total=len(journal_data_items))
        for item in journal_data_items:
            # Identifier
            item_id = journal_graphset.add_id(self.resp_agent, self.source)
            item_id.create_doi(item["DOI"])
            # BibliographicResource
            if item["type"] != "journal":
                item_br = journal_graphset.add_br(self.resp_agent, self.source)  
                ...
                DatasetBuilder._manage_volume_issue(journal_graphset, journal_br, item_br, item, self.resp_agent, self.source)
                # ResourceEmbodiment
                if "published-online" in item:
                    DatasetBuilder._manage_resource_embodiment(journal_graphset, item, item_br, True, self.resp_agent, self.source)     
                if "published-print" in item:
                    DatasetBuilder._manage_resource_embodiment(journal_graphset, item, item_br, False, self.resp_agent, self.source)     
                if "issued" in item:
            ...
            if "author" in item:
                DatasetBuilder._manage_author_ra_ar(journal_graphset, item, item_br, self.resp_agent, self.source)
            if "publisher" in item:
                publisher_ra = journal_graphset.add_ra(self.resp_agent, self.source)
                ...
                if journal_item is not None:
                    ...
                    publisher_ra.has_identifier(journal_id)
                publisher_ar = journal_graphset.add_ar(self.resp_agent, self.source)
